...
title-only completion notification, and a session changing from non-red to red
sends a title-only attention notification. Multiple sessions notify separately
instead of being combined into one summary. The initial scan at launch is ignored
so existing states do not trigger stale alerts.
